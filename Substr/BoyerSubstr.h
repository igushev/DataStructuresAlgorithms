...
int boyer_substr( const std::string& text, const std::string& pattern )
{
  ...
  while (curr >= 0) {
    while (match < pattern.size() && pattern[curr] != pattern[match]) {
      //(match - curr) to shift to matched symbols +
      //(pattern.size() - match -1) to start from the beginning
  ...
  for (int i = 0; i < pattern.size(); ++i) {
    //curr_match to shift to matched symbols +
    //(pattern.size() - i - 1) to start from the beginning
    jump[i] = std::min(jump[i], int(curr_match + pattern.size() - i - 1));
    if (curr_match == i)
